...
<template>
  <div class="min-h-screen bg-fm-white flex flex-col">
    <header class="w-full border-b border-fm-black/10">
    ...
    >
      <div v-if="open" class="fixed inset-0 z-40 bg-fm-black/40" @click="open = false" />
    </transition>
    <transition
    ...
    >
      ...
      >
        <div class="h-14 px-5 flex items-center justify-between border-b border-fm-black/10">
          <FmLogo size="sm" />
          <button class="p-2 -mr-2" :aria-label="t('nav.close')" @click="open = false">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" aria-hidden="true">
